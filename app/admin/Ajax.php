    ...
    public function cutfile_upload($action = 'token', $token ='', $data = []){
    	...
    	switch ($action) {
    		case 'token':
				...
				//判断是否存在该令牌信息
				if(! file_exists($_tokenPath . $file['token'] . '.token')){

					$file['up_size'] = 0;                       //已上传文件大小
					$pathInfo        = pathinfo($file['name']);
					$path            = $_filePath . date('Ymd') .'/';
    		...
    		case 'flash':
    			...
    			break;
    		case 'set_token_info':
    			file_put_contents($_tokenPath . $token . '.token', json_encode($data, JSON_UNESCAPED_UNICODE|JSON_NUMERIC_CHECK));
    			break;
    		case 'get_token_info':
