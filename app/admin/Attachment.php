...
//                break;
    ...
    private function saveScrawl()
    {
        ...
        $dir          = config('app.upload_path') . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . date('Ymd', $this->request->time());
    ...
    private function showFile($type, $config){
        ...
        switch ($type) {
            ...
            case 'listfile':
                ...
                $path = realpath(config('app.upload_path') .'/files/');
                break;
            /* 列出图片 */
            case 'listimage':
            default:
                ...
                $path = realpath(config('app.upload_path') .'/images/');
        }
    ...
    private function jcrop()
    {
        ...
        if ($file_path == '') {
            ...
            if (!is_dir(config('app.upload_temp_path'))) {
                mkdir(config('app.upload_temp_path'), 0766, true);
            }
            $info = $file->move(config('app.upload_temp_path'), $file->hash('md5'));
            if ($info) {
        ...
        $file_path = config('app.upload_temp_path') . str_replace(PUBLIC_PATH. 'uploads/temp/', '', $file_path);
        ...
        if (is_file($file_path)) {
            ...
            $dir_name = date('Ymd');
            $file_dir = config('app.upload_path') . DIRECTORY_SEPARATOR . 'images/' . $dir_name . '/';
            if (!is_dir($file_dir)) {
            ...
            // 水印功能
            if ($watermark == '') {
                if (config('app.upload_thumb_water') == 1 && config('app.upload_thumb_water_pic') > 0) {
                    $this->create_water($new_file_path, config('app.upload_thumb_water_pic'));
                }
            } else {
                if (strtolower($watermark) != 'close') {
                    list($watermark_img, $watermark_pos, $watermark_alpha) = explode('|', $watermark);
    ...
    {
        // 获取要生成的缩略图最大宽度和高度
        $thumb_size = $thumb_size == '' ? config('app.upload_image_thumb') : $thumb_size;
        list($thumb_max_width, $thumb_max_height) = explode(',', $thumb_size);
        // 读取图片
        $image = Image::open($file);
        // 生成缩略图
        $thumb_type = $thumb_type == '' ? config('app.upload_image_thumb_type') : $thumb_type;
        $image->thumb($thumb_max_width, $thumb_max_height, $thumb_type);
        // 保存缩略图
        $thumb_path = config('app.upload_path') . DIRECTORY_SEPARATOR . 'images/' . $dir . '/thumb/';
    ...
    private function create_water($file = '', $watermark_img = '', $watermark_pos = '', $watermark_alpha = '')
    {
        ...
        if (is_file($thumb_water_pic)) {
            ...
            $image = Image::open($file);
            // 添加水印
            $watermark_pos   = $watermark_pos   == '' ? config('app.upload_thumb_water_position') : $watermark_pos;
            $watermark_alpha = $watermark_alpha == '' ? config('app.upload_thumb_water_alpha') : $watermark_alpha;
