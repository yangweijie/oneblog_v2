...
use app\model\CrontabLog as CrontabLogModel;
...
class Autotask extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $trace         = config('trace');
        $trace['type'] = 'test';
        config($trace, 'trace');// 临时关闭TP日志写入，不然可能会出错
    ...
    public function index()
    {
        ...
        foreach ($crontab_list as $key => $crontab) {
            ...
            // 更新状态
            $crontab->where('id', $crontab['id'])->update($update);
                ...
                switch ($value['type']) {
                    case 'url':
                        if (substr($crontab['content'], 0, 1) == "/") {// 本地项目URL
                            $shell = Env::get('php_bin', '/usr/local/bin/php').' ' . Env::get('root_path').'public/index.php ' . $crontab['content'] . (IS_WIN? ' >NUL' : ' 2>&1');
                            $request = shell_exec($shell);
                            $this->saveLog('url', $value['id'], $value['title'], 1, $request);
                        } else {// 远程URL
                            try {
                                $request = \Requests::get($crontab['content']);
                                if ($request->success) {
                                    $this->saveLog('url', $value['id'], $value['title'], 1, $crontab['content'] . ' 请求成功，HTTP状态码: ' . $request->status_code);
                                } else {
                                    $this->saveLog('url', $value['id'], $value['title'], 0, $crontab['content'] . ' 请求失败，HTTP状态码: ' . $request->status_code);
                                }
                            } catch (\Requests_Exception $e) {
                                $this->saveLog('url', $value['id'], $value['title'], 0, $crontab['content'] . ' 任务异常: ' . $e->getMessage());
                            }
                        }
                        break;
                    case 'sql':
